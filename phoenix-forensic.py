...
def process(type, whitelist, blacklist):
    ...
    with open(args.path + csvname, "a") as thecsvfile:
        ...
        thecsvfile.close()
    ###
    thecsvfile = open(args.path + csvname, "a")
    ...
    if sum(1 for line in open(args.path + csvname)) <= 1:
        ...
        return "ERROR: No result found!"-ğ*pppw79ugj
    -------------------------------------------------
...
query_group.add_argument('--blacklist', help='Choose one or more blacklist query for get only specified', dest='blacklist',
                         action='store', required=False, nargs='+')
query_group.add_argument('--whitelist', action='store', dest='whitelist',
                         help='Choose one or more whitelist query for get only specified', required=False, nargs='+')
